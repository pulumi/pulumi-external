...
import (
	...
	"github.com/pulumi/pulumi-external/provider/pkg/version"
)

//go:embed cmd/pulumi-resource-external/bridge-metadata.json
var metadata []byte
...
func preConfigureCallback(vars resource.PropertyMap, c shim.ResourceConfig) error {
	...
	// _, err := stringValue(vars, "token", []string{"EXTERNAL_TOKEN"})
...
func Provider() tfbridge.ProviderInfo {
	prov := tfbridge.ProviderInfo{
		...
		Config:    map[string]*tfbridge.SchemaInfo{
			...
			//    EnvVars: []string{"EXTERNAL_MODE"}, // Multiple vars can be used
			//    Value:   "local",
			//  },
			// },
		},
		...
		Golang: &tfbridge.GolangInfo{
			ImportBasePath: path.Join(
				"github.com/pulumi/pulumi-external/sdk",
		...
		DataSources: map[string]*tfbridge.DataSourceInfo{
			"external": {
				Tok: "external:index/getExternal:getExternal",
				Docs: &tfbridge.DocInfo{
					Markdown: readFile("upstream/docs/data-sources/external.md"),
				},
			},
		},
	}
